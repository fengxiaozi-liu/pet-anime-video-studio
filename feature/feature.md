...
后续实现按“前后端并行”推进，但依赖关系仍以数据层和接口为主线控制。

- 后端泳道：任务模型、Provider 抽象、即梦接入、编排轮询、配置接口
- 前端泳道：Provider 选择与配置界面、任务状态展示、分镜聚合视图

并行推进前，先冻结三件基础契约：

- 统一任务模型：`render_jobs / scene_jobs / provider_configs`
- 统一 Provider 模板：`create_task / get_task / update_task`
- 统一前端接口：`/api/providers`、`/api/provider-configs`、任务查询接口返回聚合状态

### Todo 1：冻结基础契约

- 确认任务实体字段最终版：
  - `render_jobs` 保存父任务、故事、最终合成状态
  - `scene_jobs` 保存分镜任务、`provider_task_id`、状态、结果 URL
  - `provider_configs` 保存公共头字段和 `provider_config_json`
- 确认统一状态流转：
  - `queued`
  - `submitted`
  - `running`
...
- 重构现有 Provider 抽象，移除同步 `render(ctx)` 作为主接口
- 建立 Provider Registry
- Registry 输出三类信息：
  - 已注册 Provider
  - 已启用 Provider
  - 已配置且可用 Provider
- `provider_config_json` 统一作为私有配置入口，不再假设固定 `api_key`

### Todo 4：即梦 Provider 接入
...
- 输出最终成片
- 回写：
  - `render_jobs.final_video_url`
  - `render_jobs.final_cover_url`
- ffmpeg 在这里仅承担拼接与后处理，不承担主生成链路

### Todo 7：Provider 配置接口

- 新增 `GET /api/providers`
  - 返回已启用且已配置可选的 Provider
- 新增 `GET /api/provider-configs`
  - 返回配置状态和字段定义
- 新增 `PUT /api/provider-configs/{provider_code}`
  - 更新 `provider_config_json`
- 新增 `POST /api/provider-configs/{provider_code}/validate`
  - 校验配置合法性
- 后端返回字段定义，供前端动态渲染，不允许前端写死字段

### Todo 8：前端并行改造

- 工作区 Provider 选择器改为请求 `/api/providers`
- 移除前端硬编码 Provider 数组
...
- 配置页动态渲染：
  ...
  - `openai`：`api_key / model`
- 任务页展示父任务 + 分镜任务聚合状态
- 详情页支持查看每个分镜任务状态和最终合成状态

### Todo 9：OpenAI Provider 模板化接入

- 基于同一 BaseProvider 实现 `OpenAIProvider`
- 配置只使用：
  - `api_key`
  - `model`
  - 可选 `base_url`
- 不为 OpenAI 开特例分支，只验证模板可扩展性

### Todo 10：测试与验收

- Provider 配置校验：
  - `jimeng` 的 `app_key/app_secret`
  - `openai` 的 `api_key/model`
- 即梦任务流：
  - 创建成功写入 `provider_task_id`
  - 查询成功推进状态
  - 返回 `video_url` 后写入结果字段
- 聚合任务流：
  - 父任务创建后生成多个 `scene_jobs`
  - 所有 `scene_jobs` 完成后进入 `composing`
  - 最终生成 `final_video_url`
- 前端接口：
  - Provider 列表不再写死
  - 配置表单按后端字段定义渲染
  - 任务页能正确展示聚合状态
...
- 前端 Provider 接口：
  ...
  - `PUT /api/provider-configs/{provider_code}`
  - `POST /api/provider-configs/{provider_code}/validate`
- 即梦官方接口：
...
- 数据库存储继续按本文档里的 MySQL 方向推进
- 即梦是首个真实接入的云端 Provider，OpenAI 先作为模板验证对象
- 云厂商承担分镜片段生成，我们承担最终拼接
- 前后端并行，但后端契约优先冻结；前端不先行发明字段或状态
